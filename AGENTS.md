...
| `inference.py` | Entry point. Chunked inference, streams output to disk. Auto-selects latest checkpoint when `--weights` is omitted. |
| `tui.py` | Keyboard-navigated TUI (Rich). Primary interface, launched by `apollo.bat`/`apollo.sh`. Shows Latest and Best checkpoint options separately in inference. Ctrl+C during training saves a checkpoint and returns to menu. |
